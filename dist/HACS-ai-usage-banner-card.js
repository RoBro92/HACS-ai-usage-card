...
export function metricState(hass, metric) {
...
export class AiUsageBannerCard extends BaseHTMLElement {
  ...
  render() {
    ...
    const margin = align === "left" ? "0 auto 0 0" : align === "right" ? "0 0 0 auto" : align === "center" ? "0 auto" : "0";

    this.style.setProperty("--ai-usage-card-width", width);
    this.style.setProperty("--ai-usage-card-max-width", maxWidth);
    ...
    return `
      <section class="model" style="--accent:${escapeHtml(model.accent || "#54f2ef")}">
        ...
        <div class="rows">
          ...
          ${renderMetric(labels.weekly || "Weekly remaining", labels.weekly_reset || labels.weeklyReset || "WEEK RESET", weekly, true)}
        </div>
      </section>
    `;
  }
...
AiUsageBannerCard.styles = `
  :host {
    ...
    max-width: var(--ai-usage-card-max-width, 100%);
    margin: var(--ai-usage-card-margin, 0);
...
if (typeof window !== "undefined" && typeof HTMLElement !== "undefined") {
  ...
  const existingIndex = window.customCards.findIndex((card) => card?.type === cardPickerEntry.type);
  if (existingIndex >= 0) window.customCards.splice(existingIndex, 1, cardPickerEntry);
  else window.customCards.push(cardPickerEntry);
}
